...
const HotelBookingFlow: React.FC = () => {
  ...
  const handleSearch = async (params: HotelSearchParams) => {
    ...
    try {
      ...
      console.log('Hotel search API response:', result); // Debug: verify API response structure
      ...
      const hotels: HotelOffer[] = (result.data || []).map((offer: any, index: number) => {
          ...
          // Fix: Map price fields correctly according to HotelOffer interface
          const amount = offer.price?.amount || 0;
          const base = offer.price?.base || amount;
          const taxes = offer.price?.taxes || 0;
          const nights = offer.nights || 1;
          const perNight = offer.price?.perNight || (amount / nights);

          console.log(`Hotel ${offer.name} - amount: ${amount}, perNight: ${perNight}, nights: ${nights}`); // Debug
          ...
          return {
            ...
            rating: offer.rating || null, // Fix: rating should be number | null, not string
            ...
            price: {
              ...
              amount: amount,       // Fix: use amount (number) instead of total (string)
              perNight: perNight,   // Fix: add perNight field
              base: base,           // Fix: use number instead of string
              taxes: taxes          // Fix: use number instead of string
            },
            room: {
              type: offer.room?.type || 'STANDARD',
              description: offer.room?.description || 'Comfortable room with modern amenities',
              beds: offer.room?.beds || null,
              bedType: offer.room?.bedType || null,
              guests: offer.room?.guests || 2
            },
            amenities: offer.amenities || ['WIFI', 'RESTAURANT'],
            images: offer.images || [],
            cancellation: {
              freeCancellation: offer.cancellation?.freeCancellation || false,
              deadline: offer.cancellation?.deadline || null,
              penalty: offer.cancellation?.penalty || null
            },
            contact: offer.contact || null
          };
        });

      console.log('🏨 HotelBookingFlow - Setting search results:', hotels.length, 'hotels');
      console.log('🏨 HotelBookingFlow - First hotel:', hotels[0]);
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="text-sm text-gray-600">
                  ...
                  <p>Check-out: {searchParams?.checkOutDate || 'N/A'}</p>
                  <p className="mt-2 font-semibold text-gray-900">
                    Total: {selectedHotel.price.currency} {selectedHotel.price.amount.toFixed(2)}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                  ...
                  onClick={async () => {
                    ...
                    try {
                      await addToCart({
                        ...
                        quantity: 1,
                        price: selectedHotel.price.amount,
                        currency: selectedHotel.price.currency || 'USD',
                      });
                      openDrawer();
                      alert('Hotel added to cart successfully!');
                ...
                >
                  ...
                  Pay Now - {selectedHotel.price.currency} {selectedHotel.price.amount.toFixed(2)}
                </button>
              </div>
