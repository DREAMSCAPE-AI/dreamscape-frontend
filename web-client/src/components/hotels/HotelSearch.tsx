...
const HotelSearch: React.FC<HotelSearchProps> = ({
  ...
  const handleLocationSelect = (location: LocationSuggestion) => {
    console.log('🏨 Location selected:', location); // Debug: verify location structure
    console.log('🏨 Using city code:', location.iataCode); // Debug: verify city code value

    setLocationQuery(`${location.address.cityName}, ${location.address.countryName}`);
    setSearchParams({ ...searchParams, cityCode: location.iataCode });
    setShowLocationSuggestions(false);

    // Note: When subType='CITY', iataCode should be a city code (e.g., "NYC", "PAR")
    // If hotels are not found, verify that the API returns city codes, not airport codes
  };
