...
const HotelResults: React.FC<HotelResultsProps> = React.memo(({ hotels = [], onSelect, loading = false }) => {
  ...
  const [filters, setFilters] = useState({
    minRating: 0,
    maxPrice: 0, // FIX: 0 means no price filter by default (was 1000 which filtered out everything)
    amenities: [] as string[],
    freeCancellation: false
  });
  ...
  // Memoize the sorting and filtering logic to prevent unnecessary recalculations
  const sortedAndFilteredHotels = useMemo(() => {
    console.log('🏨 HotelResults - Input:', { hotelsCount: hotels?.length || 0, filters });

    if (!hotels || hotels.length === 0) {
      console.log('🏨 HotelResults - No hotels to filter');
      return [];
    }

    let filtered = hotels.filter(hotel => {
      // Rating filter
      if (filters.minRating > 0 && (hotel.rating || 0) < filters.minRating) {
        console.log(`🏨 Filtered out ${hotel.name} - rating too low`);
        return false;
      }

      // Price filter - use 'amount' instead of 'total'
      // FIX: Only filter if maxPrice is set (> 0)
      const price = hotel.price?.amount || 0;
      if (filters.maxPrice > 0 && price > filters.maxPrice) {
        console.log(`🏨 Filtered out ${hotel.name} - price too high: $${price} > $${filters.maxPrice}`);
        return false;
      }
      ...
      // Free cancellation filter
      if (filters.freeCancellation) {
        const hasFreeCancel = hotel.cancellation?.freeCancellation === true;
        if (!hasFreeCancel) {
          return false;
        }
      }
    ...
    console.log(`🏨 HotelResults - After filtering: ${filtered.length} hotels`);
    ...
    return filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        default:
          // Combine rating and price for recommendation score
          const scoreA = (a.rating || 0) * 0.7 + (1000 - (a.price?.amount || 0)) * 0.3;
          const scoreB = (b.rating || 0) * 0.7 + (1000 - (b.price?.amount || 0)) * 0.3;
          return scoreB - scoreA;
      }
    });
  }, [hotels, sortBy, filters]);
              ...
              <div className="lg:w-80 relative group">
                ...
                <FavoriteButton
                  ...
                  entityData={{
                    ...
                    pricePerNight: hotel.price?.perNight || 0,
                    currency: hotel.price?.currency || 'USD',
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {/* Star Rating */}
                      <div className="flex items-center">
                        {hotel.rating && hotel.rating > 0 && [...Array(Math.floor(hotel.rating))].map((_, i) => (
